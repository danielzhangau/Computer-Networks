...
def new_socket(givenPort, switch_server, mode, argc=None):
    # act as a client
    newSocket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    sys.stderr.write("new socket created to connect switch server: {}\n".format(newSocket))
    newSocket.connect((LOCALIP, int(givenPort)))
    switch_server.switch_greeting(givenPort, mode, newSocket, argc)
...
class SwitchServer:
    ...
    def switch_greeting(self, port_number, mode, newSocket=None, argc=None):
    ...
    while True:
        ...
        if mode == "global" or (mode == "local" and add_ip_address is None):
            ...
            if first_word == "connect":
                ...
                new_t = threading.Thread(target=new_socket, args=(port, switch_server, mode, len(argv)))
                new_t.start()
